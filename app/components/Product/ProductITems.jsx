...
const ProductItems = ({data}) => {
  ...
  return (
    <div className="block p-2 md:p-6 border border-[#F2F3F4] rounded-2xl w-full group relative overflow-hidden">
      ...
      <Image width={50} height={50} layout="responsive" src={data?.thumbnail} alt="product" className="w-full h-full" />
      ...
      <p className="text-sm font-normal text-secondary">
        By <span className="text-brand">{data?.brand ? data?.brand : "Nest" } </span>
      </p>
      <div className="flex flex-col md:flex-row gap-2 justify-between pt-6">
        <h4 className="text-sm md:text-lg font-bold text-brand flex gap-2">
          ${data?.price}
          <span className="text-secondary text-base line-through">${(data?.price+data?.discountPercentage).toFixed(2)}</span>
        </h4>
